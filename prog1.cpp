...
void ReadCsv(vector<Info*>& info_vector)
{
			...
			&& getline(file, VAL_DATE, ',') && getline(file, WEBSITE, ',') && getline(file, ST_SUBTYPE, '\n'))
	{
		...
		getline(file, COUNTY, ',');
		getline(file, COUNTYFIPS, ',');
		getline(file, COUNTRY, ',');

		getline(file, LATITUDE, ',');
		getline(file, LONGITUDE, ',');
		getline(file, NAICS_CODE, ',');
		getline(file, NAICS_DESC, ',');
		getline(file, SOURCE, ',');

		getline(file, SOURCEDATE, ',');
		getline(file, VAL_METHOD, ',');
		getline(file, VAL_DATE, ',');
		getline(file, WEBSITE, ',');
		getline(file, ST_SUBTYPE, '\n');
